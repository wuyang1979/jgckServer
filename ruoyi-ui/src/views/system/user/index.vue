...
export default {
      ...
      rules: {
        ...
        password: [
          { required: true, message: '用户密码不能为空', trigger: 'blur' },
          {
            min: 8,
            max: 16,
            message: '用户密码长度必须介于8和16之间！',
            trigger: 'blur',
          },
          {
            pattern:
              /^(?=.*\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*[_~!@#$%^&*])[\da-zA-Z~!@#$%^&*]{6,18}$/,
            message: '密码需要包含大小写字母、数字以及特殊符号！',
            trigger: 'blur',
          },
        ],
        email: [
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.single = selection.length != 1 || this.ids.indexOf(1) !== -1
      this.multiple = !selection.length || this.ids.indexOf(1) !== -1
    },
    // 更多操作触发
